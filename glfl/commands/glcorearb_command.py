...
def glcorearb_command():
    ...
    try:
        ...
        khronos_header_file_content = glfl.requests.downloader.download_content_from_url(glfl.requests.downloader.GLCOREARB_URL)
        glfl.utils.file_utils.write_new_binary_file("includes/GL/glcorearb.h", khronos_header_file_content)
    except Exception as exception:
        raise(exception)
    ...
    lines = glfl.utils.file_utils.read_all_lines_from_file("includes/GL/glcorearb.h")
    if(len(lines) <= 0):
    ...
    glfl.files.glfl_gl_file.write_gl_header_file(functions, "glfl-glcorearb.hpp", "glcorearb.h", "GLFL_GLCOREARB")
